...
var deepMatch = curry(function (spec, obj) {
  return compose(reduce(function (acc, _ref) {
    ...
    if (value.equals && isFunction(value.equals)) return acc && value.equals(obj[key]);else if (isRegExp(value)) return acc && value.test(obj[key]);else if (isArray(value)) return Boolean(acc && obj[key] && deepMatch(value, obj[key]));else if (isObject(value)) return Boolean(acc && obj[key] && deepMatch(value, obj[key]));
    ...
    value: function checkHeader(param) {
      if (isFunction(param)) this.headerChecks.push(param);else {
        ...
        this.headerChecks.push(function (header) {
          return compose(all(identity), map(checkProp(header)), map(function (_ref10) {
...
var Farso = function (_EventEmitter) {
  ...
  _createClass(Farso, [{
    ...
    value: function registerEndpoint(endpoint) {
      ...
      if (endpoint.use) this.router.use(endpoint.uri, endpoint.use);else {
